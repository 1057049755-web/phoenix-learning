...
(function () {
  ...
  function open(options) {
    ...
    root.innerHTML = '<div class="dialog-mask fh-ai-mask"><div class="dialog fh-ai-dialog" role="dialog" aria-modal="true" aria-labelledby="fh-ai-dialog-title">' +
      ...
      '<main class="fh-ai-main"><div class="fh-ai-main__scroll">' +
        ...
        '<form class="fh-ai-form" data-ai-form>' +
          ...
          '<section class="fh-ai-card"><div class="fh-ai-card__head"><div class="fh-ai-step">02</div><div><h4>选择具体模型</h4><p>模型名称只来自当前服务商官方目录，加载后直接展示全部可选模型。</p></div><button class="fh-ai-card-action" type="button" data-ai-models' + (!canEdit ? ' disabled' : '') + '>' + icon('refresh') + '<span>从官方读取</span></button></div><div class="fh-ai-model-toolbar"><select data-ai-model-filter aria-label="筛选模型"><option value="all">全部模型</option><option value="text">文本生成</option><option value="vision">视觉理解</option><option value="embedding">向量模型</option></select></div><div class="fh-ai-model-row"><select id="fh-ai-model" data-ai-field="model" size="8"' + (!canEdit ? ' disabled' : '') + '><option value="">选择具体模型</option></select></div><div class="fh-ai-model-foot"><p class="fh-ai-price" data-ai-model-price>价格/收费：选择具体模型后显示，免费模型也会明确标注。</p><span class="fh-ai-model-count" data-ai-model-count></span></div></section>' +
    ...
    function resetModelFilters() {
      modelFilter = 'all';
      var filter = query('[data-ai-model-filter]');
