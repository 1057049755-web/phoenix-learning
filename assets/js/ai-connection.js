...
(function () {
  ...
  function open(options) {
    ...
    var lastTest = draft && draft.lastTest ? draft.lastTest : null;
    ...
    root.innerHTML = '<div class="dialog-mask fh-ai-mask"><div class="dialog fh-ai-dialog" role="dialog" aria-modal="true" aria-labelledby="fh-ai-dialog-title">' +
      ...
      '<main class="fh-ai-main"><div class="fh-ai-main__scroll">' +
        ...
        '<form class="fh-ai-form" data-ai-form>' +
          ...
          '<section class="fh-ai-card"><div class="fh-ai-card__head"><div class="fh-ai-step">02</div><div><h4>选择具体模型</h4><p>模型名称只来自当前服务商官方目录，读取完整目录后从列表中选择。</p></div><button class="fh-ai-card-action" type="button" data-ai-models' + (!canEdit ? ' disabled' : '') + '>' + icon('refresh') + '<span>从官方读取</span></button></div><div class="fh-ai-model-toolbar"><div class="fh-ai-model-search"><span>' + icon('search') + '</span><input type="search" data-ai-model-search placeholder="搜索模型名称或 ID" aria-label="搜索模型"></div><select data-ai-model-filter aria-label="筛选模型"><option value="all">全部模型</option><option value="text">文本生成</option><option value="vision">视觉理解</option><option value="embedding">向量模型</option></select></div><div class="fh-ai-model-row"><select id="fh-ai-model" data-ai-field="model"' + (!canEdit ? ' disabled' : '') + '><option value="">选择具体模型</option></select></div><div class="fh-ai-model-foot"><p class="fh-ai-price" data-ai-model-price>价格/收费：选择具体模型后显示，免费模型也会明确标注。</p><span class="fh-ai-model-count" data-ai-model-count></span></div></section>' +
    ...
    function paintProviderMeta(providerId) {
      ...
      if (!target) return;
      if (!meta) { target.textContent = '自定义接口：请配置 Base URL、接口协议和 API Key。'; return; }
    ...
    function renderModelChoices(providerId, selected) {
      ...
      var emptyLabel = !allItems.length ? '暂无官方模型目录，请先读取模型' : (visibleItems.length ? '选择具体模型' : '没有匹配模型，请修改搜索');
      ...
      var selectedModel = selected && allItems.some(function (item) { return item.providerModelId === selected; }) ? selected : '';
      if (selectedModel && !visibleItems.some(function (item) { return item.providerModelId === selectedModel; })) options += '<option value="' + esc(selectedModel) + '">当前选择 · ' + esc(selectedModel) + '</option>';
      select.innerHTML = options;
      select.value = selectedModel;
      var current = allItems.find(function (item) { return item.providerModelId === select.value; });
      paintModelPrice(current || fallbackModel(providerId, select.value) || null);
    ...
    function draftFromForm() {
      ...
      return { id: draft.id, name: field('name').value.trim(), provider: field('provider').value, protocol: field('protocol').value, baseUrl: field('baseUrl').value.trim(), endpointPath: field('endpointPath').value.trim(), model: modelSelect.value.trim(), mode: 'direct', apiKey: field('apiKey').value.trim(), clearKey: clearKeyRequested, headers: field('headers').value.trim(), lastTest: lastTest };
    }
    ...
    function renderEditor() {
      ...
      lastTest = profile.lastTest ? { ok: profile.lastTest.ok, route: profile.lastTest.route, message: profile.lastTest.message } : lastTest; paintStatus(lastTest); query('[data-ai-foot-status]').textContent = lastTest ? (lastTest.message || '已记录最近一次检测') : '配置只保存在当前设备，可随时切换。'; paintProviderMeta(profile.provider || window.AI.DEFAULT_PROVIDER || 'zhipu');
    }
    function validate() {
      ...
      if (!value.model) errors.push({ field: 'model', message: '请从官方模型目录选择模型' });
      var stored = value.id && window.AI.getProfile ? window.AI.getProfile(value.id, true) : null;
      if (!value.apiKey && !(stored && stored.apiKey) && !clearKeyRequested) errors.push({ field: 'apiKey', message: '请填写 API Key 后再保存并启用' });
      ...
      if (errors.length) { var error = errors[0]; var target = field(error.field); if (target) target.setAttribute('aria-invalid', 'true'); var node = document.createElement('div'); node.className = 'fh-ai-error'; node.setAttribute('data-ai-error', ''); node.setAttribute('role', 'alert'); node.innerHTML = icon('info') + '<span>' + esc(error.message) + '</span>'; query('.fh-ai-main__scroll').prepend(node); node.tabIndex = -1; node.focus(); return null; }
      return value;
    }
    async function readOfficialModels() {
      ...
      try { var result = await window.AI.listModels(value); if (!result.ok) throw new Error(result.message || '官方模型列表读取失败'); localModels = result.models || []; localModelsProvider = value.provider; var candidates = [value.model, defaultModelForProvider(value.provider)].filter(Boolean); var available = modelItems(value.provider); var chosen = candidates.find(function (candidate) { return available.some(function (item) { return item.providerModelId === candidate; }); }) || ''; renderModelChoices(value.provider, chosen); query('[data-ai-foot-status]').textContent = '已从官方接口读取 ' + localModels.length + ' 个模型（2025 年至今）；请从列表选择模型。价格以接口返回或官方定价页为准。'; notify('已载入 ' + localModels.length + ' 个模型，请从列表选择', 'success'); }
    ...
    query('[data-ai-close]').addEventListener('click', close); query('[data-ai-cancel]').addEventListener('click', close); query('.fh-ai-mask').addEventListener('click', function (event) { if (event.target === event.currentTarget) close(); }); query('[data-ai-profile-search]').addEventListener('input', renderProfiles); query('[data-ai-provider]').addEventListener('change', function () { clearTestStatus(); setProviderDefaults(); }); query('[data-ai-model-search]').addEventListener('input', function () { modelSearch = this.value; renderModelChoices(field('provider').value, field('model').value); }); query('[data-ai-model-filter]').addEventListener('change', function () { modelFilter = this.value; renderModelChoices(field('provider').value, field('model').value); }); query('[data-ai-models]').addEventListener('click', readOfficialModels); query('[data-ai-field="model"]').addEventListener('change', function () { clearTestStatus(); paintModelPrice(modelItems(field('provider').value).find(function (item) { return item.providerModelId === field('model').value; }) || fallbackModel(field('provider').value, field('model').value)); }); query('[data-ai-key-toggle]').addEventListener('click', function () { var input = field('apiKey'); var shown = input.type === 'text'; input.type = shown ? 'password' : 'text'; this.setAttribute('aria-label', shown ? '显示 API Key' : '隐藏 API Key'); }); query('[data-ai-clear-key]').addEventListener('click', function () { if (!canEdit) return; clearKeyRequested = true; field('apiKey').value = ''; clearTestStatus(); query('[data-ai-key-state]').textContent = '保存时清除'; query('[data-ai-key-help]').textContent = '点击“保存并启用”后会删除此设备上的 Key；如需保留，请关闭窗口。'; }); query('[data-ai-advanced-toggle]').addEventListener('click', function () { advancedOpen = !advancedOpen; query('[data-ai-advanced]').hidden = !advancedOpen; this.setAttribute('aria-expanded', String(advancedOpen)); }); query('[data-ai-field="baseUrl"]').addEventListener('input', function () { clearTestStatus(); query('[data-ai-endpoint-preview]').textContent = endpointForDraft(draftFromForm()); }); query('[data-ai-field="protocol"]').addEventListener('change', function () { clearTestStatus(); query('[data-ai-endpoint-preview]').textContent = endpointForDraft(draftFromForm()); }); query('[data-ai-field="endpointPath"]').addEventListener('input', function () { clearTestStatus(); query('[data-ai-endpoint-preview]').textContent = endpointForDraft(draftFromForm()); }); query('[data-ai-field="apiKey"]').addEventListener('input', function () { if (this.value.trim()) clearKeyRequested = false; clearTestStatus(); }); query('[data-ai-field="name"]').addEventListener('input', function () { query('[data-ai-editor-title]').textContent = this.value.trim() || '新连接'; }); query('[data-ai-field="apiKey"]').addEventListener('blur', function () { if (this.value.trim()) window.setTimeout(readOfficialModels, 0); });
    query('[data-ai-new]').addEventListener('click', function () { if (!canEdit) return; draft = window.AI.defaultProfile(window.AI.DEFAULT_PROVIDER || 'zhipu'); selectedId = draft.id; lastTest = null; clearKeyRequested = false; localModels = []; localModelsProvider = ''; resetModelFilters(); renderEditor(); renderProfiles(); });
    query('[data-ai-delete]').addEventListener('click', function () { if (!canEdit || !selectedId || !window.AI.getProfile(selectedId)) return; if (!window.confirm('删除这组 AI 连接配置？')) return; window.AI.removeProfile(selectedId); var next = window.AI.getProfiles()[0]; if (next) { selectedId = next.id; draft = window.AI.getProfile(selectedId, false); lastTest = draft.lastTest || null; } else { selectedId = ''; draft = window.AI.defaultProfile(window.AI.DEFAULT_PROVIDER || 'zhipu'); lastTest = null; } clearKeyRequested = false; renderEditor(); renderProfiles(); notify('连接配置已删除', 'success'); });
    query('[data-ai-test]').addEventListener('click', async function () { var value = draftFromForm(); if (!value.baseUrl || !value.model) { lastTest = { ok: false, message: !value.model ? '请先从官方模型目录选择模型' : '请先检查 Base URL' }; paintStatus(lastTest); query('[data-ai-foot-status]').textContent = lastTest.message; return; } var button = query('[data-ai-test]'); button.disabled = true; button.innerHTML = icon('refresh') + '检测中…'; paintStatus({ loading: true }); try { var result = await window.AI.testProfile(value); lastTest = { ok: !!result.ok, route: result.route || 'direct', message: result.message || '' }; paintStatus(lastTest); query('[data-ai-foot-status]').textContent = lastTest.message || '检测完成'; if (lastTest.ok) notify('AI 连接测试通过', 'success'); } catch (error) { lastTest = { ok: false, route: 'direct', message: error.message || '检测失败' }; paintStatus(lastTest); query('[data-ai-foot-status]').textContent = lastTest.message; } finally { button.disabled = false; button.innerHTML = icon('check') + '检测连接'; } });
